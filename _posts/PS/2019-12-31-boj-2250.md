...
# 요약
* 트리의 부모와 자식에 대한 정보가 주어진다.
* 한 열에는 한 노드만 존재가능하다
* 왼쪽 노드는 부모노드보다 왼쪽, 오른쪽 노드는 부모노드보다 오른쪽에 존재한다.
* 모든열이 채워져야 한다.
* 각 레벨 중 가장 갭이 큰 레벨과 갭을 출력하라.

# 접근
* 중위 순회로 각 레벨의 최대 최소 높이를 구함(이걸 물어보는 문제..)
* 루트 노드를 찾음

# 풀이
1. 루트노드를 구한다. 간단하다. 입력 받은 자식들 중 체크 되지 않았다면 부모가 없는게 바로 루트이기 때문이다.
```
for (int i = 0; i < n; ++i) {
    cin >> a >> b >> c;
    tree[a].left = b;
...
```
1. 중위순회를 통해 각 레벨의 최대 최소 위치를 구한다. 원리는 간단하다. 중위순회로 방문하는 순서로 열에 하나씩 놓으면 위에 조건을 만족한다.
    ```
...
# 틀렸던 이유
* `트리의 레벨은 가장 위쪽에 있는 루트 노드가 1이고 아래로 1씩 증가한다.` 이 구문을 읽고 루트 노드가 무조건 1이라고 생각했다;; 이거도 모르고 계속 왜맞틀을 시전하고 있었다.
